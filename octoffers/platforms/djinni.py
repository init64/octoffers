...
from .chat_gpt import get_cover_letter_from_openai
...
class Djinni:
    ...
    def __init__(self, db_path="data/vacancies.db", domain='djinni.co'):
    ...
    def _get_job_list(self, url):
        ...
        self.driver_instance.driver.save_screenshot("screenshots/screenshot_vacancies.png")
    ...
    def fetch_jobs(self, url, role=None, tools=None, min_salary=None, exclusion_words=None):
        for idx in range(1, 2):
            full_url = f"{url}{self.JOB_FILTER}&primary_keyword={role}&page={idx}" if role else f"{url}&page={idx}"
            print(full_url)

            job_list = self._get_job_list(full_url)

            for job_item in job_list:
                title_element = job_item.find_element(By.CSS_SELECTOR,
                ...
                description_html = job_item.find_element(By.ID, description_element_id).get_attribute(
                    'data-original-text')
                soup = BeautifulSoup(description_html, 'lxml')
                job_description = soup.get_text(separator=' ', strip=True)

                # Retrieving salary information
                salary_element = job_item.find_elements(By.CSS_SELECTOR, 'span.public-salary-item')
                ...
                if tools and not all(tool.lower() in job_description.lower() for tool in tools):
                ...
                if exclusion_words and any(
                        word.lower() in job_title.lower() for word in
                        exclusion_words):
                    matches = False  # If exception words are found
                    print('Condition triggered exclusion_words')

                job_exists = self.db_manager.db.execute("SELECT 1 FROM jobs WHERE job_id = ?", (job_id,)).fetchone()

                if not job_exists:
                    try:
                        self.db_manager.db.execute(
                            "INSERT INTO jobs(job_id, role, link, category, source, description, salary, matches) VALUES(?, ?, ?, ?, ?, ?, ?, ?)",
                            (job_id, job_title, job_link, "my_role", "djinni", job_description, salary, matches)
                        )
    ...
    def apply_jobs(self):
        ...
        job_entries = self.db_manager.db.execute(
        ...
        for job_entry in job_entries[:1]:
            ...
            try:
                apply_button = self.wait.until(lambda driver: driver.find_element(By.CSS_SELECTOR, "button.btn.btn-primary.js-inbox-toggle-reply-form"))
                # Take and save a screenshot
                self.driver_instance.driver.save_screenshot("screenshots/screenshot_send_cv.png")
                apply_button.click()
            ...
            try:
                message_box = self.wait.until(lambda driver: driver.find_element(By.ID, "message"))
                message_box.send_keys(cover_letter)
                submit_button = self.wait.until(lambda driver: driver.find_element(By.ID, "job_apply"))
                submit_button.click()
            except TimeoutException:
            ...
            self.db_manager.db.execute("UPDATE jobs SET cv_sent = 1 WHERE job_id = ?", (job_id,))
            self.db_manager.db.commit()
